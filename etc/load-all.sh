...
do
  (echo phenotype data from PMID:$id; ./script/pombase-import.pl load-chado.yaml phenotype_annotation $HOST $DB $USER $PASSWORD < $SOURCES/pombe-embl/external_data/phaf_files/chado_load/PMID_${id}_phaf.csv) 2>&1 | tee -a $LOG_DIR/$log_file.phenotypes_from_PMID_$id.txt
done
